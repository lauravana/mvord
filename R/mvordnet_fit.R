mvordnet.fit <- function(rho){
  ...
  if (rho$evalOnly) {
    ...
    if (is.character(rho$solver)) {
      rho$optRes <- suppressWarnings(optimx(rho$start, function(x) PLfunpen(x, rho),
                                            method = rho$solver,
                                            hessian = FALSE,
                                            control = rho$control))
...
PLfunpen <- function(par, rho){
  ...
  out <- - sum(rho[["weights_fast"]] * log(c(prh, pr))) + rho$lambda *
    (rho$alpha * sum(abs(tmp[["beta"]])) + (1 - rho$alpha) * sum(tmp[["beta"]]^2))
  out
}
